...
def is_number(txt):
    pattern = r'\b([1-9]\d{0,2}(?:\.\d{3})*,\d{2})\b'
    ...
    if len(selected_boxes) == 0:
        ...
        for i, result in enumerate(array):
            ...
            for i, data_i in enumerate(data):
                ...
                arr_type.append(array_code[0] if len(array_code) > 0 else "Mboten Enten Lur")
                arr_date.append(array_date[0] if len(array_date) > 0 else "Mboten Enten Lur")
                arr_time.append(array_time[0] if len(array_time) > 0 else "Mboten Enten Lur")
                arr_cash.append(array_nominal[0] if len(array_nominal) > 0 else "Mboten Enten Lur")
                arr_saldo_akhir.append(array_nominal[1] if len(array_nominal) > 1 else "Mboten Enten Lur")
